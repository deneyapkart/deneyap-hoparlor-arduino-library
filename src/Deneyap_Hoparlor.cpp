...
@mainpage     Deneyap Speaker PAM8302A Arduino library source file
@maintainer   RFtek Electronics <techsupport@rftek.com.tr>
@version      v1.0.0
@date         August 09, 2022
@brief        Includes functions to control Deneyap Speaker PAM8302A 
              Arduino library
...
Speaker::Speaker(uint8_t TheDacPin, uint8_t TimerNo) : Speaker(TheDacPin, TimerNo, BUFFER_SIZE_DEFAULT)
{
...
Instrument_Class::Instrument_Class() : Instrument_Class(INSTRUMENT_PIANO, 127)
{
  // See main constructor routine for description of passed parameters
  // and defaults
}

Instrument_Class::Instrument_Class(int16_t InstrumentID) : Instrument_Class(InstrumentID, 127)
{
  // See main constructor routine for description of passed parameters
  // and defaults
}
...
MusicScore::MusicScore(int8_t *Score) : MusicScore(Score, TEMPO_ALLEGRO, &DEFAULT_INSTRUMENT)
{
  // set tempo and instrument to default, number of plays to 1 (once)
}

MusicScore::MusicScore(int8_t *Score, uint16_t Tempo) : MusicScore(Score, Tempo, &DEFAULT_INSTRUMENT)
{
