/*
*****************************************************************************
@file         Deneyap_Hoparlor.cpp
@mainpage     Deneyap Speaker Arduino library source file
@maintainer   RFtek Electronics <techsupport@rftek.com.tr>
@version      v1.0.2
@date         July 03, 2023
@brief        Includes functions to control Deneyap Speaker 
              Arduino library
...
uint8_t LastDacValue = 0x7f;                // Last value sent to DAC, if next is same we don't write to DAC again
PlayListItem *FirstPlayListItem = nullptr;  // first play list item to play in linked list
PlayListItem *LastPlayListItem = nullptr;   // last play list item to play in linked list

Instrument_Class DEFAULT_INSTRUMENT;
...
void Speaker::FillBuffer() {
  ...
  PlayListItem *PlayItem, *NextPlayItem;
...
uint8_t PlayListItem::NextByte() {
  return 0;
}
void PlayListItem::Init() {
}

Speaker::Speaker(uint8_t TheDacPin, uint8_t TimerNo)
  : Speaker(TheDacPin, TimerNo, BUFFER_SIZE_DEFAULT) {
  // calls other constructor as shown in function definition
}

Speaker::Speaker(uint8_t TheDacPin, uint8_t TimerNo, uint16_t PassedBufferSize) {
...
void Speaker::RemoveFromPlayList(PlayListItem *ItemToRemove) {
...
void Speaker::Play(PlayListItem *Sound) {
  Play(Sound, true);  // Sounds mix by default
}

void Speaker::Play(PlayListItem *Sound, bool Mix) {
...
bool Speaker::AlreadyPlaying(PlayListItem *Item) {
...
Wav::Wav(const unsigned char *WavData) {
...
Instrument_Class::Instrument_Class()
  : Instrument_Class(INSTRUMENT_PIANO, 127) {
  // See main constructor routine for description of passed parameters
  // and defaults
}

Instrument_Class::Instrument_Class(int16_t InstrumentID)
  : Instrument_Class(InstrumentID, 127) {
  // See main constructor routine for description of passed parameters
  // and defaults
}

Instrument_Class::Instrument_Class(int16_t InstrumentID, uint8_t Volume) {
...
void Instrument_Class::SetNote(int8_t Note) {
  this->Note = abs(Note);
}

void Instrument_Class::SetInstrument(uint16_t Instrument) {
  ...
  free(FirstEnvelope);  // remove old envelope definition
  FirstEnvelope = 0;
...
void Instrument_Class::SetDefaultInstrument() {
  SetWaveForm(WAVE_SQUARE);
}

void Instrument_Class::SetPianoInstrument() {
...
void Instrument_Class::SetHarpsichordInstrument() {
...
void Instrument_Class::SetOrganInstrument() {
...
void Instrument_Class::SetSaxophoneInstrument() {
  SetWaveForm(WAVE_SQUARE);
  FirstEnvelope = new Envelope();
  FirstEnvelope->AddPart(15, 127);
  FirstEnvelope->AddPart(3000, 0);  // An organ maintains until key released
...
void Instrument_Class::SetWaveForm(uint8_t WaveFormType) {
  // Sets the wave form for this instrument

  free(WaveForm);  // free any previous memory for a previous waveform
  switch (WaveFormType) {
    case WAVE_SQUARE:
...
void Instrument_Class::SetFrequency(uint16_t Freq) {
...
uint8_t Instrument_Class::NextByte() {
  uint8_t ByteToPlay;
...
Envelope *Instrument_Class::AddEnvelope() {
  ...
  while (ThisEnv->NextEnvelope != nullptr)
    ThisEnv = ThisEnv->NextEnvelope;
  ThisEnv->NextEnvelope = Env;
  return Env;
}

void Instrument_Class::SetDuration(uint32_t Length)  // in millis
{
...
MusicScore::MusicScore(int8_t *Score)
  : MusicScore(Score, TEMPO_ALLEGRO, &DEFAULT_INSTRUMENT) {
  // set tempo and instrument to default, number of plays to 1 (once)
}

MusicScore::MusicScore(int8_t *Score, uint16_t Tempo)
  : MusicScore(Score, Tempo, &DEFAULT_INSTRUMENT) {
  // set instrument to default and plays to 1
}

MusicScore::MusicScore(int8_t *Score, uint16_t Tempo, Instrument_Class *Instrument) {
  // Create music score
  this->Score = Score;
  this->Tempo = Tempo;
...
MusicScore::MusicScore(int8_t *Score, uint16_t Tempo, uint16_t InstrumentID) {
  // Create music score
  this->Score = Score;
  this->Tempo = Tempo;
...
void MusicScore::SetInstrument(uint16_t InstrumentID) {
  Instrument->SetInstrument(InstrumentID);
}
...
Envelope::~Envelope() {
  // free any further linked envelopes and associated envelope parts for all envelopes found
  // Note if you free from an envelope that it is not the start envelope then the previous
  // envelopes will still exist and in fact the last one before the one you free will still
  // point to this one. If for some reason you wish to free a none first envelope then you
  // must tidy up the last of the previous envelopes yourself.

  EnvelopePart *Part, *NextPart;

  if (NextEnvelope != 0)
    free(NextEnvelope);  // if another envelope in the chain free this also

  // free the parts for this envelope
  Part = FirstPart;
  while (Part != 0) {
...
uint8_t Envelope::NextByte(uint8_t ByteToPlay) {
...
void Envelope::InitEnvelopePart(EnvelopePart *EPart, uint8_t LastVolume) {
...
EnvelopePart *Envelope::AddPart(uint16_t Duration, uint16_t StartVolume, uint16_t TargetVolume) {
  EnvelopePart *EPart;
...
EnvelopePart *Envelope::AddPart(uint16_t Duration, uint16_t TargetVolume) {
  // creates and adds an envelope part to this current envelope
  EnvelopePart *EPart = new EnvelopePart();
...
// Envelope part class
void EnvelopePart::SetDuration(uint16_t Duration) {
  this->Duration = Duration;
  this->RawDuration = 50 * Duration;
}

uint16_t EnvelopePart::GetDuration() {
  return Duration;
}
...
void SquareWave::Init(int8_t Note) {
  if (Note != -1)             // use the note not a raw frequency
    Frequency = Notes[Note];  // if -1 then use the raw frequency
  if (Frequency > 25000)
    Frequency = 25000;
...
uint8_t TriangleWave::NextByte() {
  // returns the next byte for this frequency of a square wave
  // called at the raw sample rate
  NextAmplitude += ChangeAmplitudeBy;
  if (NextAmplitude > 255) {                 // top of a peak, reverse direction
    ChangeAmplitudeBy = -ChangeAmplitudeBy;  // reverse direction
    NextAmplitude = 255;
  } else {
    if (NextAmplitude < 0) {                   // bottom of a trough, reverse direction
      ChangeAmplitudeBy = -ChangeAmplitudeBy;  // reverse direction
      NextAmplitude = 0;
    }
  }
...
void TriangleWave::Init(int8_t Note) {
  NextAmplitude = 127;
  if (Note != -1)             // use the note not a raw frequency
    Frequency = Notes[Note];  // if -1 then use the raw frequency
...
void SawToothWave::Init(int8_t Note) {
  NextAmplitude = 0;
  if (Note != -1)             // use the note not a raw frequency
    Frequency = Notes[Note];  // if -1 then use the raw frequency
...
void SineWave::Init(int8_t Note) {
  CurrentAngle = 0;
  if (Note != -1)             // use the note not a raw frequency
    Frequency = Notes[Note];  // if -1 then use the raw frequency
...
void Sequence::AddPlayItem(PlayListItem *PlayItem) {
  ...
  SequenceItem_Class *SequenceItem;
  // create a new sequence item
  SequenceItem = new SequenceItem_Class();
...
void Sequence::RemoveAllPlayItems() {
  // Remove all items in linked list and free associated memory

  SequenceItem_Class *ThisItem, *NextItem;
...
FilterNoise::FilterNoise(int8_t AmountOfNoise) {
  this->Min = -AmountOfNoise;
  this->Max = AmountOfNoise;
}

FilterNoise::FilterNoise(int8_t Min, int8_t Max) {
  this->Min = Min;
  this->Max = Max;
}

uint8_t FilterNoise::FilterWave(uint8_t TheByte) {
